...
@Service
public class TaskGgqzdBizImpl implements ITaskBaseBiz {
    ...
    public ActivityInfoDto getActivityInfo() {
        return new ActivityInfoDto().setActivityCode(ActivityBiz.ACTIVITY_GGQZD)
                .setCustomerIdVsMobile(2);
    }

    @Override
    public String getCustomerIdAndCheckDoTaskAuth(ActTaskReqDto req, String activityCode,
                                                  String taskCode, String mobile, ActTask actTask) {
        //region 校验有没有完成该任务权限; 并获取 List<Map<String, Object>> vasOrderList
        Date now = new Date();
        Map<String, Object> pMap = new HashMap<>();
        pMap.put("mobile", mobile);
        pMap.put("goodsCode", "GGQZD");
