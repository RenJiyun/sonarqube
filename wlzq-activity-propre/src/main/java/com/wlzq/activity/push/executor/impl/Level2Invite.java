 ...
 */
@Slf4j
@Service("Level2Invite")
public class Level2Invite extends BaseExecutorTask {
	
	@Autowired
	private Level2RecieveInviteDao level2RecieveInviteDao;
	@Autowired
	private PushBusinessRecordDao pushBusinessRecordDao;
	
	private static final String MSGTHEMECODE = "Level2Invite";
	
	private static final String LOG_TAG = "邀请好友注册任务";
	
	
	private static final String DATE_FORMATE = "yyyy-MM-dd";
	
	private static final String SMS_LINK = "m.wlzq.cn/c?d4";
	private static final String SMS_CONTENT = "恭喜您获得炒股利器Level-2增强行情1个月免费体验资格，详情点 " + SMS_LINK + " 拒收回T";
	...
	public StatusDto run() {
		...
		List<Level2RecieveInvite> users = this.getLevel2RecieveInvite(yesterday);
	...
	private List<Level2RecieveInvite> getLevel2RecieveInvite(Date date) {
		Level2RecieveInvite qry = new Level2RecieveInvite();
		qry.setStatus(1);
		qry.setCreateTimeBegin(DateUtils.getDayStart(date));
		qry.setCreateTimeEnd(DateUtils.getDayEnd(date));
		List<Level2RecieveInvite> list = level2RecieveInviteDao.findList(qry);
		return list;
	}
	
	private void handleUsers(List<Level2RecieveInvite> users) {
...
//						}
			...
			for (Level2RecieveInvite user : users) {
				orderSort++;
//				String openId = accUserBind.getThirdUid();
//				String customerId = user.getCustmerId();
				String mobile = user.getInviteMobile();
				String uniKey = null;
				uniKey = mobile.trim();
				
				Map<String, Object> busParams = Maps.newHashMap();
				busParams.put("mobile", mobile);
				busParams.put("targetType", PushTargetType.MOBILE.getTargetType());
				busParams.put("targetCode", mobile);
...
//				record.setCustomerName(user.get);
				...
				log.info(new StringBuilder().append(LOG_TAG).append("<|>").append("生成单条推送数据").append("<|>")
						...
						.append("targetType:").append(PushTargetType.MOBILE.getTargetType()).append("<|>")
						.append("targetCode:").append(mobile).append("<|>")
